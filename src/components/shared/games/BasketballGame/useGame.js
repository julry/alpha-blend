            ...
            try {
                ...
                const game = {
                    ...
                    createGameElements() {
                        ...
                        // Создаем кольцо
                        this.elements.hoop = {
                            x: this.width / 2 - 118 * ratio,
                            y: this.height - 495 * ratio,
                            width: 236 * ratio,
                            height: 150
                        };
                        ...
                        this.elements.rim = {
                            left: { x: this.width / 2 - (60 * ratio), y: this.height - 373 * ratio, radius: 3 * ratio },
                            right: { x: this.width / 2 + (54 * ratio), y: this.height - 373 * ratio, radius: 3 * ratio }
                        };

                        // Границы корзины для столкновений
                        this.elements.basketBounds = {
                            left: this.width / 2 - 60 * ratio,
                            right: this.width / 2 + 57 * ratio,
                            top: this.height - (373 - 4) * ratio ,
                            bottom: this.height - (373 + 4) * ratio 
                        };
                        ...
                        if (ball.launched) {
                            ...
                            if (ball.velocity.y > 0 && prevY <= rimBottom && ball.y > rimBottom && !ball.isBelowHoop) {
                                ...
                                if (ball.x > this.elements.rim.left.x && ball.x < this.elements.rim.right.x + this.elements.rim.right.radius) {
                                    // Засчитываем очки
                                    setCurrentScore(prev => prev + GAME_POINTS);
                                } else {
                                    onMiss?.();
                                }
                            }
                    ...
                    render() {
                        ...
                        if (frontRimImg) {
                            const leftPoint = this.elements.rim.left; 
                            const rightPoint = this.elements.rim.right; 
                            ctx.drawImage(frontRimImg, leftPoint.x, leftPoint.y, rightPoint.x - leftPoint.x + leftPoint.radius * 2, leftPoint.radius * 2);
                        }
                        // Рисуем обод
                        const rimImg = this.images.side_rim;
                        if (rimImg) {
                            ctx.drawImage(rimImg, this.elements.rim.left.x, this.elements.rim.left.y);
                            ctx.drawImage(rimImg, this.elements.rim.right.x, this.elements.rim.right.y);
                        }
                        ...
                        if (this.elements.ball.velocity.y > 0) {
                            if (frontRimImg) {
                                const leftPoint = this.elements.rim.left; 
                                const rightPoint = this.elements.rim.right; 
                                ctx.drawImage(frontRimImg, leftPoint.x, leftPoint.y, rightPoint.x - leftPoint.x, leftPoint.radius);
                            }
                        }
                    },
                    gameLoop(timestamp) {
